...
static puint64 WINAPIpp_time_profiler_get_hr_ticks (void);
  ...
  pp_time_profiler_get_hr_ticks (void)
{
  ...
  if (P_UNLIKELY (QueryPerformanceCounter(&tcounter) == FALSE)) {
    P_ERROR (
      "PTimeProfiler::pp_time_profiler_get_hr_ticks: QueryPerformanceCounter() failed");
    tcounter.QuadPart = 0;
  }
...
p_time_profiler_init(void) {
  ...
  if (has_qpc == TRUE) {
    if (P_UNLIKELY (QueryPerformanceFrequency(&tcounter) == 0)) {
      P_ERROR (
        "PTimeProfiler::p_time_profiler_init: QueryPerformanceFrequency() failed");
      has_qpc = FALSE;
    } else {
      pp_time_profiler_freq = (puint64) (tcounter.QuadPart);
      pp_time_profiler_ticks_func =
        (PWin32TicksFunc) pp_time_profiler_get_hr_ticks;
      pp_time_profiler_elapsed_func =
        (PWin32ElapsedFunc) pp_time_profiler_elapsed_hr;
    }
  }
  ...
  if (P_UNLIKELY (has_qpc == FALSE)) {
    ...
    if (P_UNLIKELY (hmodule == NULL)) {
      P_ERROR (
        "PTimeProfiler::p_time_profiler_init: failed to load kernel32.dll module");
      return;
    }
    ...
    pp_time_profiler_elapsed_func =
      (PWin32ElapsedFunc) pp_time_profiler_elapsed_tick64;
