...
static void pp_uthread_get_os2_priority(PUThreadPriority prio, PULONG thr_class,
  PLONG thr_level);
static PULONG pp_uthread_get_tls_key(PUThreadKey *key);
static void pp_uthread_clean_destructors(void);
static void pp_uthread_os2_proxy(ptr_t data);

static void
pp_uthread_get_os2_priority(PUThreadPriority prio, PULONG thr_class,
  PLONG thr_level) {
  switch (prio) {
    case P_UTHREAD_PRIORITY_INHERIT: {
      ...
      if (P_UNLIKELY (DosGetInfoBlocks(&ptib, NULL) != NO_ERROR)) {
        P_WARNING (
          "PUThread::pp_uthread_get_os2_priority: DosGetInfoBlocks() failed");
...
pp_uthread_get_tls_key(PUThreadKey *key) {
  ...
  if (P_LIKELY (thread_key == NULL)) {
    ...
    if (P_UNLIKELY (DosAllocThreadLocalMemory(1, &thread_key) != NO_ERROR)) {
      P_ERROR (
        "PUThread::pp_uthread_get_tls_key: DosAllocThreadLocalMemory() failed");
      p_free(destr);
      p_mutex_unlock(pp_uthread_tls_mutex);
      return NULL;
    }
    ...
    if (destr != NULL) {
      ...
      if (P_UNLIKELY (p_atomic_pointer_compare_and_exchange(
        ...
        if (P_UNLIKELY (DosFreeThreadLocalMemory(thread_key) != NO_ERROR))
          P_ERROR (
            "PUThread::pp_uthread_get_tls_key: DosFreeThreadLocalMemory() failed");
