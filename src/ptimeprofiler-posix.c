...
#endif
    P_ERROR (
      "PTimeProfiler::pp_time_profiler_get_ticks_clock: clock_gettime() failed");
...
pp_time_profiler_get_ticks_gtod() {
  ...
  if (P_UNLIKELY (gettimeofday(&tv, NULL) != 0)) {
    P_ERROR (
      "PTimeProfiler::pp_time_profiler_get_ticks_gtod: gettimeofday() failed");
    return 0;
  }
...
p_time_profiler_init(void) {
  ...
  if (P_LIKELY (sysconf(_SC_MONOTONIC_CLOCK) > 0))
    ...
    pp_time_profiler_ticks_func =
      (PPOSIXTicksFunc) pp_time_profiler_get_ticks_gtod;
#else
