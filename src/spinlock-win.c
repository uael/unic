...
u_spinlock_lock(spinlock_t *spinlock) {
  ...
  while (u_atomic_int_compare_and_exchange(&(spinlock->spin), 0, 1) == false);
  return true;
}
...
u_spinlock_trylock(spinlock_t *spinlock) {
  ...
  return u_atomic_int_compare_and_exchange(&(spinlock->spin), 0, 1);
}
...
u_spinlock_unlock(spinlock_t *spinlock) {
  ...
  u_atomic_int_set(&(spinlock->spin), 0);
  return true;
}
