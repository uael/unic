...
u_condvar_new(void) {
  ...
  if (U_UNLIKELY (cond_init(&ret->hdl, NULL, NULL) != 0)) {
...
u_condvar_free(condvar_t *cond) {
  ...
  if (U_UNLIKELY (cond_destroy(&cond->hdl) != 0))
    U_WARNING ("condvar_t::u_condvar_free: cond_destroy() failed");
  u_free(cond);
}
...
u_condvar_wait(condvar_t *cond,
  ...
  if (U_UNLIKELY (cond_wait(&cond->hdl, (mutex_t *) mutex) != 0)) {
    U_ERROR ("condvar_t::u_condvar_wait: cond_wait() failed");
    return false;
  }
  return true;
}
...
u_condvar_signal(condvar_t *cond) {
  ...
  if (U_UNLIKELY (cond_signal(&cond->hdl) != 0)) {
    U_ERROR ("condvar_t::u_condvar_signal: cond_signal() failed");
    return false;
  }
  return true;
}
...
u_condvar_broadcast(condvar_t *cond) {
  ...
  if (U_UNLIKELY (cond_broadcast(&cond->hdl) != 0)) {
    U_ERROR (
      "condvar_t::u_condvar_broadcast: cond_broadcast() failed");
    return false;
  }
  return true;
}
