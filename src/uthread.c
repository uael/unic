...
p_uthread_init(void) {
  if (P_LIKELY (pp_uthread_specific_data == NULL)) {
    pp_uthread_specific_data =
      p_uthread_local_new((destroy_fn_t) pp_uthread_cleanup);
  }
...
uthread_t *
p_uthread_create_full(uthread_fn_t func,
  ptr_t data,
  bool joinable,
  uthread_prio_t prio,
  size_t stack_size) {
  ...
  p_spinlock_lock(pp_uthread_new_spin);
  base_thread = (PUThreadBase *) p_uthread_create_internal(
    pp_uthread_proxy,
    joinable,
    prio,
    stack_size
...
uthread_t *
p_uthread_create(uthread_fn_t func,
  ptr_t data,
  bool joinable) {
  /* All checks will be inside */
  return p_uthread_create_full(
    func, data, joinable, P_UTHREAD_PRIORITY_INHERIT,
    0
  );
}
...
p_uthread_exit(int code) {
  ...
  if (P_UNLIKELY (base_thread->ours == false)) {
    P_WARNING (
      "uthread_t::p_uthread_exit: p_uthread_exit() cannot be called from an unknown thread");
    return;
  }
...
p_uthread_current(void) {
  ...
  if (P_UNLIKELY (base_thread == NULL)) {
    if (P_UNLIKELY ((base_thread = p_malloc0(sizeof(PUThreadBase))) == NULL)) {
      P_ERROR ("uthread_t::p_uthread_current: failed to allocate memory");
      return NULL;
    }
...
p_uthread_ideal_count(void) {
  ...
  if (P_UNLIKELY (sys_info_func == NULL)) {
    P_ERROR (
      "uthread_t::p_uthread_ideal_count: failed to get address of system info procedure");
    return 1;
  }
  ...
  if (P_UNLIKELY (cores < 0)) {
    P_WARNING ("uthread_t::p_uthread_ideal_count: failed to call sysconf(_SC_NPROC_ONLN)");
    cores = 1;
  }
  ...
  if (P_UNLIKELY (sysctl (mib, 2, &cores, &len, NULL, 0) == -1)) {
    P_WARNING ("uthread_t::p_uthread_ideal_count: failed to call sysctl()");
    return 1;
  }
...
# endif
  ...
  if (P_UNLIKELY (!$VMS_STATUS_SUCCESS (status))) {
    P_WARNING ("uthread_t::p_uthread_ideal_count: failed to call lib$get_ef()");
    return 1;
  }
  ...
  if (P_UNLIKELY (!$VMS_STATUS_SUCCESS (status))) {
    P_WARNING ("uthread_t::p_uthread_ideal_count: failed to call sys$getsyiw()");
    lib$free_ef (&efn);
    return 1;
  }
  ...
  if (P_UNLIKELY (!$VMS_STATUS_SUCCESS (status))) {
    P_WARNING ("uthread_t::p_uthread_ideal_count: failed to call tis_synch()");
    lib$free_ef (&efn);
    return 1;
  }

  if (P_UNLIKELY (iosb.iosb$l_getxxi_status != SS$_NORMAL)) {
    P_WARNING ("uthread_t::p_uthread_ideal_count: l_getxxi_status is not normal");
    lib$free_ef (&efn);
    return 1;
  }
  ...
  if (P_UNLIKELY (DosQuerySysInfo (QSV_NUMPROCESSORS,
    ...
    P_WARNING ("uthread_t::p_uthread_ideal_count: failed to call DosQuerySysInfo()");
    return 1;
  }
  ...
  if (P_UNLIKELY (get_system_info_v (&sys_info, SYS_INFO_VERSION) != 0)) {
    P_WARNING ("uthread_t::p_uthread_ideal_count: failed to call get_system_info_v()");
    return 1;
  }
  ...
  if (P_UNLIKELY (__scoinfo (&utsn, sizeof (utsn)) == -1)) {
    P_ERROR ("uthread_t::p_uthread_ideal_count: failed to call __scoinfo()");
    return 1;
  }
  ...
  if (P_UNLIKELY (cores == -1)) {
    P_WARNING (
      "uthread_t::p_uthread_ideal_count: failed to call sysconf(_SC_NPROCESSORS_ONLN)");
    return 1;
  }
