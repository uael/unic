...
p_condvar_free(p_condvar_t *cond) {
  ...
  if (P_UNLIKELY (pthread_cond_destroy(&cond->hdl) != 0))
    P_WARNING (
      "p_condvar_t::p_condvar_free: pthread_cond_destroy() failed");
...
p_condvar_wait(p_condvar_t *cond,
  ...
  if (P_UNLIKELY (
    ...
    P_ERROR ("p_condvar_t::p_condvar_wait: pthread_cond_wait() failed");
    return false;
  }
...
p_condvar_signal(p_condvar_t *cond) {
  ...
  if (P_UNLIKELY (pthread_cond_signal(&cond->hdl) != 0)) {
    P_ERROR (
      "p_condvar_t::p_condvar_signal: pthread_cond_signal() failed");
    return false;
  }
...
p_condvar_broadcast(p_condvar_t *cond) {
  ...
  if (P_UNLIKELY (pthread_cond_broadcast(&cond->hdl) != 0)) {
    P_ERROR (
      "p_condvar_t::p_condvar_broadcast: thread_cond_broadcast() failed");
    return false;
  }
