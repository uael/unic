 ...
 * u_atomic_is_lock_free(). Otherwise usage of spinlocks will be the same as the
 * ordinary mutex.
 *
 * To create a new spinlock primitive the u_spinlock_new() routine should be
 * called, to delete the unused spinlock primitive use u_spinlock_free().
 *
 * Use u_spinlock_lock() or u_spinlock_trylock() to synchronize access at the
 * beginning of the critical section. Only the one thread is allowed to pass
 * this call, others will wait for the u_spinlock_unlock() call which marks the
 ...
 */
#ifndef U_SPINLOCK_H__
# define U_SPINLOCK_H__
 ...
 *
 * If the atomic model is not lock-free this call will have the same effect
 * as u_mutex_unlock(), thus it is not safe to call this routine on an unlocked
 * spinlock.
 */
U_API bool
