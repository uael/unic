 ...
 * u_condvar_new(), create and lock a mutex before a critical section and
 * wait for a signal from another thread on this condition variable
 * using u_condvar_wait().
 *
 * The signaling thread behavior: upon reaching event time emit a signal with
 * u_condvar_signal() to wake up a single waiting thread or
 * u_condvar_broadcast() to wake up all the waiting threads.
 *
 ...
 */
#ifndef U_CONDVAR_H__
# define U_CONDVAR_H__
 ...
 */
U_API bool
u_condvar_wait(condvar_t *cond, mutex_t *mutex);
 ...
 * u_condvar_wait() while there are other waiting threads.
 */
U_API bool
