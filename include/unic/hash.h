 ...
 * - GOST (R 34.11-94).
 *
 * Use u_crypto_hash_new() to initialize a new hash context with one of the
 * mentioned above types. Data for hashing can be added in several chunks using
 * the u_crypto_hash_update() routine. You can add more chunks as long as the
 * hash context is open.
 *
 * The hash context becomes close in two cases: u_crypto_hash_get_string() or
 * u_crypto_hash_get_digest() was called. After that you can only get a hash in
 ...
 * @note After calling u_crypto_hash_get_string() or u_crypto_hash_get_digest()
