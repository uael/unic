...
static void *
semaphore_test_thread(void *) {
  sema_t *sem;
  int_t i;

  sem = p_sema_new("p_semaphore_test_object", 1, P_SEMA_OPEN, NULL);
...
BOOST_AUTO_TEST_CASE (psemaphore_nomem_test) {
  ...
  BOOST_CHECK (
    p_sema_new("p_semaphore_test_object", 1, P_SEMA_CREATE, NULL) == NULL);
...
BOOST_AUTO_TEST_CASE (psemaphore_general_test) {
  ...
  BOOST_CHECK (p_sema_new(NULL, 0, P_SEMA_CREATE, &error) == NULL);
  BOOST_CHECK (error != NULL);
  clean_error(&error);

  BOOST_REQUIRE (p_sema_acquire(sem, &error) == false);
  BOOST_CHECK (error != NULL);
  clean_error(&error);

  BOOST_REQUIRE (p_sema_release(sem, &error) == false);
  BOOST_CHECK (error != NULL);
  clean_error(&error);
  ...
  sem = p_sema_new("p_semaphore_test_object", 10, P_SEMA_CREATE, NULL);
  ...
  for (i = 0; i < 10; ++i)
    BOOST_CHECK (p_sema_acquire(sem, NULL));

  for (i = 0; i < 10; ++i)
    BOOST_CHECK (p_sema_release(sem, NULL));
...
BOOST_AUTO_TEST_CASE (psemaphore_thread_test) {
  ...
  BOOST_REQUIRE (p_sema_acquire(sem, NULL) == false);
  BOOST_REQUIRE (p_sema_release(sem, NULL) == false);
  p_sema_free(sem);
  p_sema_take_ownership(sem);
