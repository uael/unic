...
static int
test_hash_table_values(const_ptr_t a, const_ptr_t b) {
...
BOOST_AUTO_TEST_CASE (phashtable_general_test) {
  ...
  BOOST_CHECK (
    PPOINTER_TO_INT(p_hash_table_lookup(table, PINT_TO_POINTER(1))) == 15);
  BOOST_CHECK (
    PPOINTER_TO_INT(p_hash_table_lookup(table, PINT_TO_POINTER(2))) == 20);
  BOOST_CHECK (
    PPOINTER_TO_INT(p_hash_table_lookup(table, PINT_TO_POINTER(3))) == 30);
  ...
  BOOST_REQUIRE (
    PPOINTER_TO_INT(p_hash_table_lookup(table, PINT_TO_POINTER(22))) == 20);
...
BOOST_AUTO_TEST_CASE (phashtable_stress_test) {
  ...
  while (counter != PHASHTABLE_STRESS_COUNT) {
    ...
    if (p_hash_table_lookup(table, PINT_TO_POINTER (rand_number))
      != (ptr_t) (-1))
      continue;
    ...
    p_hash_table_insert(table, PINT_TO_POINTER (keys[counter]),
  ...
  for (int i = 0; i < PHASHTABLE_STRESS_COUNT; ++i) {
    ...
    BOOST_CHECK (
      p_hash_table_lookup(table, PINT_TO_POINTER(keys[i])) == (ptr_t) (-1));
  }
  ...
  while (counter != PHASHTABLE_STRESS_COUNT) {
    ...
    p_hash_table_insert(table, PINT_TO_POINTER (rand_number),
