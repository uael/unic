...
tree_complexity(tree_t *tree) {
  ...
  switch (p_tree_get_type(tree)) {
    ...
    case P_TREE_TYPE_RB:
      return 2
        * ((int_t) (log((double) p_tree_get_nnodes(tree) + 1) / log(2.0)));
    case P_TREE_TYPE_AVL: {
      double phi = (1 + sqrt(5.0)) / 2.0;
      return (int_t) (log(sqrt(5.0) * (p_tree_get_nnodes(tree) + 2)) / log(phi)
        - 2);
    }
    default:
      return p_tree_get_nnodes(tree);
...
stress_tree_test(tree_t *tree, int node_count) {
  ...
  while (counter != node_count) {
    ...
    p_tree_insert(tree, PINT_TO_POINTER (keys[counter]),
...
BOOST_AUTO_TEST_CASE (ptree_nomem_test) {
  ...
  for (int i = (int) P_TREE_TYPE_BINARY; i <= (int) P_TREE_TYPE_AVL; ++i) {
    tree_t *tree = p_tree_new((tree_kind_t) i, (cmp_fn_t) compare_keys);
    ...
    BOOST_CHECK (p_tree_new((tree_kind_t) i, (cmp_fn_t) compare_keys) == NULL);
    p_tree_insert(tree, PINT_TO_POINTER (1), PINT_TO_POINTER (10));
    BOOST_CHECK (p_tree_get_nnodes(tree) == 0);
...
BOOST_AUTO_TEST_CASE (ptree_invalid_test) {
  ...
  for (int i = (int) P_TREE_TYPE_BINARY; i <= (int) P_TREE_TYPE_AVL; ++i) {
    /* Invalid usage */
    BOOST_CHECK (p_tree_new((tree_kind_t) i, NULL) == NULL);
    BOOST_CHECK (p_tree_new((tree_kind_t) -1, (cmp_fn_t) compare_keys) == NULL);
    BOOST_CHECK (p_tree_new((tree_kind_t) -1, NULL) == NULL);

    BOOST_CHECK (p_tree_new_with_data((tree_kind_t) i, NULL, NULL) == NULL);
    BOOST_CHECK (
      p_tree_new_with_data((tree_kind_t) -1, (cmp_data_fn_t) compare_keys, NULL)
        == NULL);
    BOOST_CHECK (p_tree_new_with_data((tree_kind_t) -1, NULL, NULL) == NULL);

    BOOST_CHECK (p_tree_new_full((tree_kind_t) i,
      NULL,
      NULL,
      NULL,
      NULL) == NULL);
    BOOST_CHECK (p_tree_new_full((tree_kind_t) -1,
    ...
    BOOST_CHECK (p_tree_get_type(NULL) == (tree_kind_t) -1);
...
BOOST_AUTO_TEST_CASE (ptree_general_test) {
  ...
  for (int i = (int) P_TREE_TYPE_BINARY; i <= (int) P_TREE_TYPE_AVL; ++i) {
    ...
    BOOST_CHECK (
      general_tree_test(tree, (tree_kind_t) i, false, false) == true);
    ...
    BOOST_CHECK (general_tree_test(tree, (tree_kind_t) i, true, false) == true);
    ...
    BOOST_CHECK (general_tree_test(tree, (tree_kind_t) i, true, true) == true);
