...
static int_t read_count = 0;
static int_t write_count = 0;

#ifndef P_OS_HPUX
volatile static bool is_working = false;

static void *
shm_buffer_test_write_thread(void *) {
  ...
  while (is_working == true) {
    ...
    ssize_t op_result = p_shm_buffer_get_free_space(buffer, NULL);
    ...
    op_result =
      p_shm_buffer_write(buffer, (ptr_t) test_str, sizeof(test_str), NULL);
...
static void *
shm_buffer_test_read_thread(void *) {
  shmbuf_t *buffer = p_shm_buffer_new("pshm_test_buffer", 1024, NULL);
  ...
  while (is_working == true) {
    ...
    ssize_t op_result = p_shm_buffer_get_used_space(buffer, NULL);
    ...
    op_result =
      p_shm_buffer_read(buffer, (ptr_t) test_buf, sizeof(test_buf), NULL);
...
BOOST_AUTO_TEST_CASE (pshmbuffer_general_test) {
  ...
  BOOST_CHECK (
    p_shm_buffer_write(buffer, (ptr_t) test_str, sizeof(test_str), NULL)
      == sizeof(test_str));
  BOOST_CHECK (
    p_shm_buffer_get_free_space(buffer, NULL) == (1024 - sizeof(test_str)));
  BOOST_CHECK (p_shm_buffer_get_used_space(buffer, NULL) == sizeof(test_str));
  BOOST_CHECK (
    p_shm_buffer_read(buffer, (ptr_t) test_buf, sizeof(test_buf), NULL)
      == sizeof(test_str));
  BOOST_CHECK (
    p_shm_buffer_read(buffer, (ptr_t) test_buf, sizeof(test_buf), NULL) == 0);
  ...
  BOOST_CHECK (p_shm_buffer_write(buffer, (ptr_t) large_buf, 2048, NULL) == 0);
...
BOOST_AUTO_TEST_CASE (pshmbuffer_thread_test) {
  ...
  thr1 =
    p_uthread_create((uthread_fn_t) shm_buffer_test_write_thread, NULL, true);
  BOOST_REQUIRE (thr1 != NULL);

  thr2 =
    p_uthread_create((uthread_fn_t) shm_buffer_test_read_thread, NULL, true);
  BOOST_REQUIRE (thr1 != NULL);
