...
BOOST_AUTO_TEST_CASE (psocketaddress_nomem_test) {
  ...
  BOOST_CHECK (
    p_socketaddr_to_native(sock_addr, addr_buf, native_size - 1) == false);
  BOOST_CHECK (
    p_socketaddr_to_native(sock_addr, addr_buf, native_size) == true);
  ...
  if (p_socketaddr_is_ipv6_supported()) {
    sock_addr6 =
      p_socketaddr_new("2001:cdba:345f:24ab:fe45:5423:3257:9652", 1058);
    ...
    BOOST_CHECK (
      p_socketaddr_to_native(sock_addr6, addr_buf6, native_size6) == true);
  ...
  if (p_socketaddr_is_ipv6_supported()) {
    BOOST_CHECK (
      p_socketaddr_new_from_native(addr_buf6, native_size6 - 1) == NULL);
    p_free(addr_buf6);
  }
