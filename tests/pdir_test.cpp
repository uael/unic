...
BOOST_AUTO_TEST_CASE (pdir_nomem_test) {
  ...
  BOOST_CHECK (p_dir_new(PDIR_TEST_DIR
    "/", NULL) == NULL);
...
BOOST_AUTO_TEST_CASE (pdir_general_test) {
  ...
  BOOST_CHECK (strcmp(orig_path, PDIR_TEST_DIR
    "/") == 0);
  p_free(orig_path);
